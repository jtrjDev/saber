        ...
        <form action="{{ route('ferramentas.store') }}" method="POST" enctype="multipart/form-data">
            ...
            {{-- Estado --}}
               <x-ui.select name="estado" label="Estado" required>
                    <option value="">Selecione…</option>
                    <option value="bom" @selected(old('estado') === 'bom')>Bom</option>
                    <option value="ruim" @selected(old('estado') === 'ruim')>Ruim</option>
                    <option value="manutenção" @selected(old('estado') === 'manutenção')>Manutenção</option>
                    <option value="quebrado" @selected(old('estado') === 'quebrado')>Quebrado</option>
                </x-ui.select>
